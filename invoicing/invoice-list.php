            ...
            <div class="card bg-white">
                <div class="card-content">
                    <div class="card-body">
                        ...
                        <a href="invoice-create.php?IQ=I"><span class="la la-plus" data-toggle="tooltip" data-placement="top" title="Create New"></span></a>
                        </h4>
                        <?php
						$total_pages = $mysqli->query('SELECT * FROM invoices WHERE invoice_type = "Invoice"')->num_rows;
						...
						if ($stmt = $mysqli->prepare('SELECT * 
							...
							$result = $stmt->get_result();?>
									...
									<tr>
										...
										<?if($date < $now & $row['status'] != "Paid"){
											print '<td class="text text-danger">' . $row['invoice_due_date'] . '</td>';
										} else {
											print '<td class="text">' . $row['invoice_due_date'] . '</td>';
										}?>
									...
									<?	if($row['status'] == "Open" || $row['status'] == "Draft" || $row['status'] == "Sent"){	?>
										...
										<td>
										<a href="includes/functions.php?action=paid&pre=<? echo $row["invoice_pre"]?>&no=<? echo $row["invoice_no"]?>" class="btn btn-success btn-sm py-0" style="font-size: 0.8em;">
										...
										&bal=<? echo $row["total"]?>&dd=<? echo $row["invoice_due_date"]?>&ir=<? echo $row["invoice_reference"]?>&co=<? echo $row["company"]?>&p=1"
										...
										<a href="qtinvdcs/Invoice-<? echo $row["invoice_pre"].$row["invoice_no"]?>.pdf" class="btn btn-info btn-sm py-0" style="font-size: 0.8em;" target="_blank">
									...
									<?	} elseif ($row['status'] == "Paid"){?>
										<td><b><span class="text text-success"><? echo $row['status']?></span></b></td>

												<td>
												<a href="includes/functions.php?action=open&pre=<? echo $row["invoice_pre"]?>&no=<? echo $row["invoice_no"]?>" class="btn btn-danger btn-sm py-0" style="font-size: 0.8em;">
													<span class="la la-undo" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Mark Open"></span></a>

													<a href="includes/functions.php?action=paidinqt&pre=<? echo $row["invoice_pre"]?>&no=<? echo $row["invoice_no"]?>&e1=<? echo $row["custom_email"]?>&e2=<? echo $row["custom_email_1"]?>
													&bal=<? echo $row["total"]?>&dd=<? echo $row["invoice_due_date"]?>&ir=<? echo $row["invoice_reference"]?>&co=<? echo $row["company"]?>&p=1"
													class="btn btn-warning btn-sm py-0" style="font-size: 0.8em; email-invoice">
									...
									<ul class="pagination">
										<?php if ($page > 1): ?>
										<li class="prev"><a href="invoice-list.php?page=<?php echo $page-1 ?>">Prev</a></li>
										<?php endif; ?>

										<?php if ($page > 3): ?>
										<li class="start"><a href="invoice-list.php?page=1">1</a></li>
										<li class="dots">...</li>
										<?php endif; ?>

										<?php if ($page-2 > 0): ?><li class="page"><a href="invoice-list.php?page=<?php echo $page-2 ?>"><?php echo $page-2 ?></a></li><?php endif; ?>
										<?php if ($page-1 > 0): ?><li class="page"><a href="invoice-list.php?page=<?php echo $page-1 ?>"><?php echo $page-1 ?></a></li><?php endif; ?>

										<li class="currentpage"><a href="invoice-list.php?page=<?php echo $page ?>"><?php echo $page ?></a></li>

										<?php if ($page+1 < ceil($total_pages / $num_results_on_page)+1): ?><li class="page">
											<a href="invoice-list.php?page=<?php echo $page+1 ?>"><?php echo $page+1 ?></a></li><?php endif; ?>
										<?php if ($page+2 < ceil($total_pages / $num_results_on_page)+1): ?><li class="page">
											<a href="invoice-list.php?page=<?php echo $page+2 ?>"><?php echo $page+2 ?></a></li><?php endif; ?>

										<?php if ($page < ceil($total_pages / $num_results_on_page)-2): ?>
										<li class="dots">...</li>
										<li class="end"><a href="invoice-list.php?page=<?php echo ceil($total_pages / $num_results_on_page) ?>">
										<?php echo ceil($total_pages / $num_results_on_page) ?></a></li>
										<?php endif; ?>

										<?php if ($page < ceil($total_pages / $num_results_on_page)): ?>
										<li class="next"><a href="invoice-list.php?page=<?php echo $page+1 ?>">Next</a></li>
										<?php endif; ?>
									</ul>
									<?php endif;
							 $stmt->close();
